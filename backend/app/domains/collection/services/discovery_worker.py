...
def _resolve_job_actor_and_olt(db: Session, job_id: UUID) -> tuple[Actor, UUID | None]:
    """Resolve ator humano (se houver) e olt_id do job para o audit_log.

    Se `requested_by_user_id` for NULL (jobs disparados pelo Celery beat,
    por exemplo), devolve `system_actor()`. Se o job desapareceu, também
    devolve system_actor + olt_id None (audit segue como best-effort)."""
    row = db.execute(
        text(
            """
            SELECT j.olt_id, j.requested_by_user_id, u.username
            FROM collection_job j
            LEFT JOIN app_user u ON u.app_user_id = j.requested_by_user_id
            WHERE j.collection_job_id = :id
            """
        ),
        {"id": str(job_id)},
    ).first()
    ...
    olt_id, user_id, username = row
    if user_id is None:
        return system_actor(), olt_id
    return (
...
def _mark_failed(job_id: UUID, error_message: str) -> None:
    ...
    Best-effort: se a própria escrita (ou o audit_log) falhar, o worker
    não deve travar. Falha aqui vira log e o job pode ficar em RUNNING
    até ser recuperado pelo detect_stale_jobs."""
    try:
        with session_scope() as db:
            actor, olt_id = _resolve_job_actor_and_olt(db, job_id)
            db.execute(
                text(
                    """
            ...
            audit_record_sync(
                db,
                actor=actor,
                action=AuditAction.COLLECTION_JOB_FINISHED,
                result=AuditResult.FAILURE,
                entity_type="collection_job",
                entity_id=job_id,
                olt_id=olt_id,
                error_detail=error_message[:1000],
                after={"status": JobStatus.FAILED.value},
            )
    except Exception as exc:
        log.exception(
...
_JOB_STATUS_TO_AUDIT_RESULT: dict[JobStatus, AuditResult] = {
    JobStatus.SUCCESS: AuditResult.SUCCESS,
    JobStatus.PARTIAL: AuditResult.PARTIAL,
}
...
def _mark_finished(job_id: UUID, status: JobStatus, *, error_message: str | None) -> None:
    ...
    with session_scope() as db:
        ...
        audit_record_sync(
            ...
            actor=actor,
            action=AuditAction.COLLECTION_JOB_FINISHED,
            result=_JOB_STATUS_TO_AUDIT_RESULT.get(status, AuditResult.SUCCESS),
            entity_type="collection_job",
            entity_id=job_id,
            olt_id=olt_id,
