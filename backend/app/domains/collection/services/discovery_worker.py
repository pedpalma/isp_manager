...
def _mark_running(order_id: UUID) -> None:
    ...
    Só grava audit se a transição realmente aconteceu (rowcount > 0);
    se o worker perdeu a corrida (ordem foi cancelada, por exemplo),
    o UPDATE condicional não afeta linha e não há evento a auditar."""
    with session_scope() as db:
        result = cast(
            CursorResult,
            db.execute(
