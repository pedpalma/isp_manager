...
# - NÃO chama commit/rollback. Quem decide o limite da transação é o Service. O repositório só faz add/flush e queries.
# - Levanta erros do SQLAlchemy (IntegrityError etc.); o Service traduz para erros de domínio.
...
class ManufacturerRepository:
    ...
    async def list_page(
        self, *, offset: int, limit: int, only_active: bool = False, search: str | None = None
        ...
        Para tabelas de catálogo (poucas linhas) é aceitável."""
    ...
    async def add(self, manufacturer: Manufacturer) -> None:
        """Adiciona à sessão e força flush para popular o `manufacturer_id` gerado pelo Postgres. NÃO commita: quem decide é o Service."""
        self._session.add(manufacturer)
        await self._session.flush()

    async def flush(self) -> None:
        """Atalho para refletir mudanças pendentes (UPDATEs feitos manipulando os atributos do objeto carregado pela sessão)."""
        await self._session.flush()
